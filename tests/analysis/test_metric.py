...
from fa.analysis.metric import Metric
from fa.archive.load import FINANCIAL_REPORT_TYPES


class TestMetric(unittest.TestCase):
    def test_from_archive(self):
        with patch("fa.analysis.metric.load_historical_data", MagicMock(return_value='h')) as mock_load_historical_data, \
             patch("fa.analysis.metric.load_financial_data_all", MagicMock(return_value={rt: rt[0] for rt in FINANCIAL_REPORT_TYPES})) as mock_load_financial_data_all, \
             patch("fa.analysis.metric.Metric.__init__", MagicMock(return_value=None)) as mock_constructor:
