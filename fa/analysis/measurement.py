...
def get_first_commonly_available_year(timeseries_coll, availability=1):
	""" Returns the earliest year (integer) at which <availability> proportion of all timeseries in <timeseries_coll> has a
		numeric value, returns None if not found.
	...
	for year in range(start_date.year, end_date.year + 1):
		...
		for ts in timeseries_coll:
			values = ts.get(str(year))

			if values is not None and not np.isnan(values).all() and next(counter) >= min_total_available_values:
				return year
